...
Documentation for releases of this gem can be found [on RubyDoc](https://gemdocs.org/gems/privy_ruby).
...
privy_api = Privy::Client.new(
  app_id: ENV["PRIVY_APP_ID"], # This is the default and can be omitted
  app_secret: ENV["PRIVY_APP_SECRET"], # This is the default and can be omitted
  environment: "staging" # defaults to "production"
)

wallet = privy_api.wallets.get("wallet_id")
...
When the library is unable to connect to the API, or if the API returns a non-success status code (i.e., 4xx or 5xx response), a subclass of `Privy::Errors::APIError` will be thrown:
...
rescue Privy::Errors::RateLimitError => e
...
privy_api.wallets.get("wallet_id", request_options: {max_retries: 5})
```
...
# Or, configure per-request:
privy_api.wallets.get("wallet_id", request_options: {timeout: 5})
```

On timeout, `Privy::Errors::APITimeoutError` is raised.
...
All parameter and response objects inherit from `Privy::Internal::Type::BaseModel`, which provides several conveniences, including:
...
puts(wallet[:my_undocumented_property])
```
...
The `Privy::Client` instances are threadsafe, but are only are fork-safe when there are no in-flight HTTP requests.

Each instance of `Privy::Client` has its own HTTP connection pool with a default size of 99. As such, we recommend instantiating the client once per application in most settings.
...
```ruby
privy_api.wallets.get("wallet_id")
```
...
params = Privy::WalletGetParams.new
privy_api.wallets.get("wallet_id", **params)
```
...
See [the contributing documentation](https://github.com/privy-io/ruby-sdk/tree/main/CONTRIBUTING.md).
